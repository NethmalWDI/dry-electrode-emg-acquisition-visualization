...
def main():
    ...
    while running:
        ...
        # 1. Points
        points = []
        # Draw from left to right. Buffer has newest at right.
        # We want newest at SCREEN_WIDTH, oldest at 0? 
        # Or standard scrolling: buffer[0] at x=0? 
        # collections.deque with maxlen appends to right. So buffer[-1] is newest.
        # If we draw index 0 at x=0, the graph moves left.
        
        # Adjust so that we fill the width.
        # If buffer length < SCREEN_WIDTH, just draw what we have.
        # If we resized and buffer is smaller/bigger, we iterate what we have.
